...
let rec map_tree_k (f : 'a -> 'b) (t : 'a tree) (k : 'b tree -> 'c) : 'c =
  | Leaf -> k Leaf
  | Node (l,x,r) -> map_tree_k f l (fun y -> map_tree_k f r (fun z -> Node ( y,f x,z ))
...
let rec cchange (coins: int list) (amt:int) (fc: unit -> int list) = 
    ...
    if coin > amt then
      change cs amt fc
        ...
        cchange coins (amt - coin) (fun x -> fc (coin::x))
      with Change -> change cs amt
    end
...
type lock_object = {open: unit -> unit; close: unit -> unit}
let make_lock  = 
  let lock = ref 0 in 
  if 
